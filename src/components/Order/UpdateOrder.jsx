...
const UpdateOrder = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const order = await fetchResource(`orders/${id}`);
        ...
        setLoading(false);
        // Populate the form with existing order details
        setFormData({
          ...order,
  ...
  const handleSubmit = async (e) => {
    ...
    await updateResource(`orders/${id}`, updatedFormData);
      ...
      {!loading && (
        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <Link
              to={`/orders/${id}`}
              className="bg-red-400 hover:bg-red-500 text-white font-bold py-2 px-4 rounded outline-none"
            >
              Cancel
            </Link>
          </div>
